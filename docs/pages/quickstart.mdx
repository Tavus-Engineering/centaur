...
direct-message scopes such as `im:history`. Scheduled task delivery to a user's
DM requires `im:write`.

With message-event subscriptions enabled, replies still need to mention the bot.
Unmentioned replies are collected as context for the next addressed turn.
