/**
 * Watch Agent usage guide for the Slack App Home tab, ported from slackbot v1's
 * `slack/home.ts` (dropped in the v2 migration). Rendered below the
 * integration-heartbeat board by integration-health.ts.
 */

export function usageGuideBlocks(): unknown[] {
  return [
    divider(),
    header('Watch Agent'),
    section(
    ...
    fields([
      '*Harness selectors*\nDefault is Codex. Add `--codex`, `--amp`, or `--claude` when you want a specific runtime.',
      '*Repo and shell tools*\n`git`, `gh`, `rg`, `fd`, `jq`, `uv`, `node`, `bun`, `pnpm`, `rust`, `tmux`, `cmake`, and protobuf tooling.',
      '*Centaur helper*\nInside the sandbox, API tools are called with `call <tool> <method> <json>`.',
    ...
    section(
      [
        ...
        '`signoz-generating-queries`, `signoz-investigating-alerts`, `signoz-creating-dashboards`, `signoz-creating-alerts`, plus dashboard, alert, view, docs, and ClickHouse-query helpers.'
      ].join('\n')
    ),
    divider(),
    ...
    section(
      [
        ...
        '- `Investigate <a Slack permalink> and figure out what is responsible.` Then react :white_check_mark: on the follow-up prompt to post the answer back to that thread.',
...
function fields(items: string[]): unknown {
  return { type: 'section', fields: items.map(text => ({ type: 'mrkdwn', text })) }
}
