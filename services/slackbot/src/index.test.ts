...
describe('Slack event HTTP dedupe', () => {
  ...
  it('keeps addressed channel-thread mentions inline with one investigating reaction', async () => {
    process.env.SLACK_SIGNING_SECRET = 'test-signing-secret'
    ...
    try {
      const { app } = await import(`./index.ts?thread_inline=${Date.now()}`)
      ...
      expect(slackCalls.some(call => call.path === '/api/conversations.open')).toBe(false)
      expect(slackCalls.some(call => call.path === '/api/chat.postMessage')).toBe(false)
      ...
      expect(JSON.stringify(workflow?.input)).not.toContain('route')
      expect(JSON.stringify(workflow?.input)).not.toContain('DM')
    } finally {
      await slackApi.stop()
