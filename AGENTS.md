...
This repository is Tavus's fork of
[paradigmxyz/centaur](https://github.com/paradigmxyz/centaur). Every pull
request against this fork must add exactly one line to [FORK.md](FORK.md) with
the PR head commit hash, a one-line description, and the PR URL. Reviewers
should block pull requests that omit the record.

Validate changes on the local Kubernetes stack before pushing. For a runtime
change, build the affected service with `just build-one <service>`, deploy it
locally with `just deploy`, and make a real request through the changed path.
Do not use the production deploy host as a development or end-to-end test
environment.

When the user explicitly asks to deploy the Tavus fork, finish local validation,
commit on a branch, add the required `FORK.md` entry, push the branch, and run
the checked-in `make deploy` wrapper from the production checkout on
`samuel-l40s`. The production checkout is `~/projects/centaur`; fetch the Tavus
fork explicitly when its `origin` points upstream. Set
`KUBECONFIG=/etc/rancher/k3s/k3s.yaml`, keep Centaur isolated in its existing
k3s namespace, and do not stop or repurpose non-Centaur containers on the
shared host. Do not replace `make deploy` with an ad hoc Helm upgrade unless
the wrapper is broken and the user approves the fallback.
